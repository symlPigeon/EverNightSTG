...
pub fn collision_system(
    ...
    let hurtboxes: Vec<HurtEntry> = storage
        ...
        .map(|(id, hb)| {
            ...
            HurtEntry { entity: id, aabb, shape: world_shape, layer: hb.layer.as_u32() }
        })
        .collect();
    ...
    let mut hits: Vec<(EntityId, EntityId, Vec2, Vec2)> = {
        ...
        for (att_id, att_shape_local, att_group, att_hit_once, tf) in &raw_hitboxes {
            ...
            for &hurt_idx in &candidates {
                if visited[hurt_idx] == generation { continue; }
                visited[hurt_idx] = generation;
                let hurt = &hurtboxes[hurt_idx];
                if *att_id == hurt.entity { continue; }
                if !att_aabb.overlaps(&hurt.aabb) { continue; }
    ...
    let mut hits: Vec<(EntityId, EntityId, Vec2, Vec2)> = {
        ...
        raw_hitboxes
            ...
            .fold(
                ...
                || (Vec::<(EntityId, EntityId, Vec2, Vec2)>::new(), Vec::<usize>::new(), vec![0u32; n_hurt], 0u32),
    ...
    for (attacker, defender, contact_point, normal) in hits {
        event_bus.push(EventPayload::Collision { attacker, defender, contact_point, normal, tick });
    }
}
